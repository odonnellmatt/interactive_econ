  ...
  const Slider = ({ label, value, min, max, step, stateKey }: { label: string, value: number, min: number, max: number, step: number, stateKey: keyof AppState }) => (
    <div className="mb-3">
        ...
        <div className="flex items-center gap-1">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (!isNaN(val)) handleChange(stateKey, val);
            }}
      ...
      <input
        ...
        step={step}
        value={value}
        onChange={(e) => handleChange(stateKey, parseFloat(e.target.value))}
  ...
  return (
    <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200">
      ...
      {(state.model === 'standard' || state.model === 'elasticity') && (
        <>
          <Slider label="Shift Demand" value={state.shiftD} min={-40} max={40} step={1} stateKey="shiftD" />
          <Slider label="Shift Supply" value={state.shiftS} min={-40} max={40} step={1} stateKey="shiftS" />
        </>
      )}

      {state.model === 'elasticity' && (
        <>
          <Slider label="Demand Slope (Elasticity)" value={state.slopeD} min={-3} max={-0.2} step={0.1} stateKey="slopeD" />
          <Slider label="Supply Slope (Elasticity)" value={state.slopeS} min={0.2} max={3} step={0.1} stateKey="slopeS" />
          <Slider label="Per-Unit Tax" value={state.tax} min={0} max={40} step={1} stateKey="tax" />
        </>
      )}
